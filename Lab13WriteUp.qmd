...
- The mean for the further condition is negative (−0.213), suggesting that dopamine levels decreases when the birds sing further away from their adult song

- The mean for the closer condition is positive (0.179), suggesting that dopamine levels tend increases when the birds sing closer to their adult song

- The mean for difference is positive (0.391), showing that dopamine levels are generally higher when birds sing closer compared to further away
...
```{r}
#| size: scriptsize
library(effectsize) # load effect size library

# t-tests
closer<- t.test(x = fig.dat$Closer, mu = 0, alternative = "greater") # increases when singing closer- right tailed

further<- t.test(x = fig.dat$Further, mu = 0, alternative = "less") # decreases when singing further- left tailed

difference<- t.test(x = fig.dat$Difference, mu = 0, alternative = "two.sided") # differences btw closer and further- two sided
n<- nrow(fig.dat) # number of values
...
(difference_t.stat<- mean(fig.dat$Difference)/(sd(fig.dat$Difference)/sqrt(n))) #for differences
